...
class HorseArmorTypeTransformer implements ITransformer {
    ...
    public void transform(ClassNode node, boolean dev) {
        node.methods.stream().filter(methodNode -> methodNode.name.equals(dev ? "getByItem" : "func_188576_a")).forEachOrdered(methodNode -> {
            ...
            inject.add(new TypeInsnNode(INSTANCEOF, "com/mcmoddev/lib/api/item/IHorseArmor"));
            final LabelNode label1 = new LabelNode();
            inject.add(new JumpInsnNode(IFEQ, label1));
            inject.add(new VarInsnNode(ALOAD, 0));
            inject.add(new TypeInsnNode(CHECKCAST, "com/mcmoddev/lib/api/item/IHorseArmor"));
            inject.add(new MethodInsnNode(INVOKEINTERFACE, "com/mcmoddev/lib/api/item/IHorseArmor", "getArmorType",
