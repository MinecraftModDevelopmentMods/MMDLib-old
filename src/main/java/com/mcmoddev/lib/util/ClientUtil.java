...
public class ClientUtil {
    ...
    public static void setCustomModelLocation (Block block) {
        final ModelResourceLocation location = new ModelResourceLocation(block.getRegistryName(), "inventory");
    ...
    public static void renderFluidCuboid (FluidStack fluid, BlockPos pos, double x, double y, double z, double w, double h, double d) {
        final double wd = (1d - w) / 2d;
        final double hd = (1d - h) / 2d;
        final double dd = (1d - d) / 2d;
    ...
    public static void renderFluidCuboid (FluidStack fluid, BlockPos pos, double x, double y, double z, double x1, double y1, double z1, double x2, double y2, double z2, int color) {
        ...
        renderer.begin(GL11.GL_QUADS, DefaultVertexFormats.BLOCK);
        Minecraft.getMinecraft().renderEngine.bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
        final int brightness = Minecraft.getMinecraft().world.getCombinedLight(pos, fluid.getFluid().getLuminosity());
    ...
    public static void renderStackedFluidCuboid (FluidStack fluid, double px, double py, double pz, BlockPos pos, BlockPos from, BlockPos to, double ymin, double ymax) {
        ...
        final int xd = to.getX() - from.getX();
        final int yd = (int) (ymax - ymin);
        final int zd = to.getZ() - from.getZ();
        final double xmin = fluidOffset;
        final double xmax = xd + 1d - fluidOffset;
        final double zmin = fluidOffset;
        final double zmax = zd + 1d - fluidOffset;
        ...
        for (int y = 0; y <= yd; y++)
            for (int z = 0; z <= zd; z++)
                for (int x = 0; x <= xd; x++) {
                    final double x1 = xs[x];
                    final double x2 = xs[x + 1] - x1;
                    final double y1 = ys[y];
                    final double y2 = ys[y + 1] - y1;
                    final double z1 = zs[z];
                    final double z2 = zs[z + 1] - z1;
                    if (x == 0)
    ...
    public static void putTexturedQuad (VertexBuffer renderer, TextureAtlasSprite sprite, double x, double y, double z, double w, double h, double d, EnumFacing face, int r, int g, int b, int a, int light1, int light2, boolean flowing) {
        ...
        while (yt2 > 1f)
            yt2 -= 1f;
        final double zt1 = z1 % 1d;
        double zt2 = zt1 + d;
        while (zt2 > 1f)
            zt2 -= 1f;
        if (flowing) {
            final double tmp = 1d - yt1;
            yt1 = 1d - yt2;
            yt2 = tmp;
        }
        switch (face) {
